...
describe("Payload Helpers", function () {
    ...
    it("should detect binary data in object", function () {
        const asserts: Array<[any, boolean]> = [
            [
                { foo : "bar" },
                false
            ],
    ...
    it("should create FormData object from raw data object", function () {
        ...
        const names = arrayChunk((formData as any)._streams, 3)
            .map(([name]: string[]) => name.match(/name=\"(.*)\"/i)[1]);

        expect(names).to.be.array().containingAllOf([
            "foo",
            "foo1.bar",
            "foo2.0",
            "foo3.0.bar",
