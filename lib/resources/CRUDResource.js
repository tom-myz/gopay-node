...
var CRUDResource = (function (_super) {
    ...
    CRUDResource.prototype.defineRoute = function (method, path) {
        ...
        return function route(pathParams, data, callback, options) {
            ...
            var validator = new Validator(data, schema, error_codes_1.validationCodes);
            if (validator.fails()) {
            ...
            return api.send(["GET", "DELETE"].indexOf(method) !== -1 ? req.query(data) : req.send(data), callback, options.token);
        };
    };
    return CRUDResource;
}(WithAPI_1.WithAPI));
